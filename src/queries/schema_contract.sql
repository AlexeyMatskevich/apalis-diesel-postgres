...
WITH expected_columns(table_name, column_name, type_name, required) AS (VALUES
 ...
 WHERE (NOT $2 OR (e.table_name != 'queue_stats_snapshot' AND e.column_name != 'lease_token'))
 AND (a.attname IS NULL OR format_type(a.atttypid, a.atttypmod) != e.type_name
      OR (a.attnotnull != e.required AND NOT ($2 AND e.column_name = 'priority'))
 ...
 ('jobs_list_by_queue_idx', CASE WHEN $1
 ...
 WHERE NOT $2 AND (i.indexrelid IS NULL OR NOT i.indisvalid OR NOT i.indisready
 ...
 WHERE NOT $2 AND (p.oid IS NULL OR p.prosecdef OR p.proretset != e.returns_set
     ...
     OR p.proconfig IS NULL OR NOT (p.proconfig @> ARRAY['search_path=pg_catalog, apalis, pg_temp']
                  OR (NOT $1 AND p.proconfig @> ARRAY['search_path=pg_catalog, apalis'])))
), trigger_problems AS (
 SELECT 'trigger notify_workers' AS problem WHERE NOT $2 AND NOT EXISTS (
